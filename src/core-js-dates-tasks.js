...
function getNextFriday(date) {
  ...
  const daysUntilFriday =
    currentDayOfWeek === 5 ? 7 : (5 + 7 - currentDayOfWeek) % 7;
  const nextFridayTimestamp =
    date.getTime() + daysUntilFriday * oneDayInMilliseconds;
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil(((today - firstDay) / 86400000 + firstDay.getDay() + 1) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startDate = new Date(changeDateFormate(period.start));
  const endDate = new Date(changeDateFormate(period.end));
  const schedule = [];
